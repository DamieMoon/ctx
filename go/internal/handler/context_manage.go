...
// adminTier classifies how a manage or blob-manage action is gated (MT T25,
// 05-A8). It replaces the binary actionRequiresAdmin (052/G03) with the
// two-tier cut of design 05 §4.4: server-global actions stay server-admin, the
// per-tenant key actions become tenant-admin-capable.
type adminTier int

const (
	// tierUnset is the ZERO VALUE and must never appear in a dispatch table
	// or on the wire. It exists because the zero value used to be tierOpen:
	// a table row written without a tier: field then compiled and admitted
	// every valid key silently — the worst possible direction for a
	// forgotten field. With tierUnset first, both enforcers can SEE the
	// omission and refuse it (500 + slog.Error, never a 403: a 403 would
	// tell the caller a tier exists). Defence in depth, not a policy: every
	// shipped row carries a tier, pinned by the table gates.
	tierUnset       adminTier = iota
	tierOpen                  // no admin gate (auth + scope only)
